...
def update_historical_performance_and_vendor_metrics(sender,instance,**kwargs):
    ...
    if instance.status == PurchaseOrderStatus.completed:
        ...
        data.update({"on_time_delivery_rate":on_time_delivery_rate})
    ...
    if instance.quality_rating is not None:
        ...
        data.update({"quality_rating_avg":quality_rating_avg})
    ...
    if instance.acknowledgment_date is not None:
        ...
        data.update({"average_response_time":average_response_time})
    ...
    data.update({"fulfillment_rate":fulfillment_rate})

    print(data)

    HistoricalPerformance.objects.update_or_create(vendor=vendor_obj,defaults=data)

    vendor_obj.on_time_delivery_rate = on_time_delivery_rate if data.get("on_time_delivery_rate") is not None else 0
    vendor_obj.fulfillment_rate = fulfillment_rate
    vendor_obj.average_response_time = average_response_time if data.get("average_response_time") is not None else 0
    vendor_obj.quality_rating_avg = quality_rating_avg if data.get("quality_rating_avg") is not None else 0
    vendor_obj.save()
